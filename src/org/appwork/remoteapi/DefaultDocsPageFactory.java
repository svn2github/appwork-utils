...
public class DefaultDocsPageFactory extends InterfaceHandler<Object> {
        ...
        if (requiredTypes.size() > 0) {
            ...
            for (Type enumClass : lst) {
                if (enumClass instanceof Class && !((Class) enumClass).isEnum()) {
                    ...
                    ClassCache cc;
                    try {
                        cc = ClassCache.getClassCache(((Class) enumClass));
                        ...
                        int widestKey = 0;
                        for (Setter sg : cc.getSetter()) {
                            keySet.add(sg.getKey());
                            widestKey = Math.max(sg.getKey().length(), widestKey);
                        }
                        for (Getter sg : cc.getGetter()) {
                            keySet.add(sg.getKey());
                            widestKey = Math.max(sg.getKey().length(), widestKey);
                        }
    ...
    private List<Type> getTypes(Type returnType, HashSet<Type> dupe) {
        ...
        if (returnType instanceof Class) {
            ...
            if (!isAddObjectToHelp(returnType)) {
                return ret;
            }
            ret.add(returnType);
            ClassCache cc;
            try {
                cc = ClassCache.getClassCache((Class<? extends Object>) returnType);
                for (Getter g : cc.getGetter()) {
                    ret.addAll(getTypes(g.getMethod().getGenericReturnType(), dupe));
                }
                for (Setter g : cc.getSetter()) {
