    ...
    private void parse() {
        ...
        while (clazz != null && clazz != RemoteCallInterface.class) {
            for (final Method m : clazz.getDeclaredMethods()) {
                if (!dupe.add(m.getName())) {
                    throw new InterfaceParseException("Method " + m.getName() + " is avlailable twice in " + clazz);
                }
                try {
                    ...
                    for (final Type t : m.getGenericParameterTypes()) {
                        JSonStorage.canStore(t, (AllowNonStorableObjects) null);
                    }
