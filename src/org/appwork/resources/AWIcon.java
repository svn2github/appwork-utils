/**
 *
 * ====================================================================================================================================================
 *         "AppWork Utilities" License
 *         The "AppWork Utilities" will be called [The Product] from now on.
 * ====================================================================================================================================================
 *         Copyright (c) 2009-2015, AppWork GmbH <e-mail@appwork.org>
 *         Schwabacher Straße 117
 *         90763 Fürth
 *         Germany
 * === Preamble ===
 *     This license establishes the terms under which the [The Product] Source Code & Binary files may be used, copied, modified, distributed, and/or redistributed.
 *     The intent is that the AppWork GmbH is able to provide  their utilities library for free to non-commercial projects whereas commercial usage is only permitted after obtaining a commercial license.
 *     These terms apply to all files that have the [The Product] License header (IN the file), a <filename>.license or <filename>.info (like mylib.jar.info) file that contains a reference to this license.
 *
 * === 3rd Party Licences ===
 *     Some parts of the [The Product] use or reference 3rd party libraries and classes. These parts may have different licensing conditions. Please check the *.license and *.info files of included libraries
 *     to ensure that they are compatible to your use-case. Further more, some *.java have their own license. In this case, they have their license terms in the java file header.
 *
 * === Definition: Commercial Usage ===
 *     If anybody or any organization is generating income (directly or indirectly) by using [The Product] or if there's any commercial interest or aspect in what you are doing, we consider this as a commercial usage.
 *     If your use-case is neither strictly private nor strictly educational, it is commercial. If you are unsure whether your use-case is commercial or not, consider it as commercial or contact as.
 * === Dual Licensing ===
 * === Commercial Usage ===
 *     If you want to use [The Product] in a commercial way (see definition above), you have to obtain a paid license from AppWork GmbH.
 *     Contact AppWork for further details: e-mail@appwork.org
 * === Non-Commercial Usage ===
 *     If there is no commercial usage (see definition above), you may use [The Product] under the terms of the
 *     "GNU Affero General Public License" (http://www.gnu.org/licenses/agpl-3.0.en.html).
 *
 *     If the AGPL does not fit your needs, please contact us. We'll find a solution.
 * ====================================================================================================================================================
 * ==================================================================================================================================================== */
package org.appwork.resources;

import java.awt.Image;

import javax.swing.Icon;

/**
 * @author thomas
 * @date 14.10.2016
 *
 */
public enum AWIcon implements IconRef {
    close("close"),
    copy("copy"),
    cut("cut"),
    delete("delete"),
    paste("paste"),
    select("select"),
    dev("dev"),
    popDownSmall("popDownSmall"),
    popUpSmall("popUpSmall"),
    download("download"),
    EXTFILESYSTEM_fileIcon("fileIcon"),
    DIALOG_CANCEL("dialog/cancel"),
    FILE_SYSTEM_ROOT("filesystem_root"),
    EXTFILESYSTEM_folder("folder"),
    MIME_fileIcon("fileIcon"),
    DIALOG_LOGIN("dialog/login"),
    DIALOG_FIND("dialog/find"),
    DIALOG_INFO("dialog/info"),
    LOCKPANEL_WAIT("wait"),
    TABLE_COLUMN_COMBO_popUpLarge("popUpLarge"),
    TABLE_COLUMN_COMBO_popDownLarge("popDownLarge"),
    TABLE_COLUMN_BUTTON("exttable/columnButton"),
    TABLE_WIDTH_LOCKED("exttable/widthLocked"),
    TABLE_SORT_ASC("exttable/sortAsc"),
    TABLE_SORT_DESC("exttable/sortDesc"),
    TABLE_SEARCH_DIALOG_FIND("dialog/find"),
    FILECHOOSER_FOLDER("folder"),
    FILECHOOSER_DESKTOP("desktop"),
    FILECHOOSER_NETWORK("network"),
    FILECHOOSER_HARDDRIVE("drive-harddisk"),
    FILECHOOSER_FILE("fileIcon"),
    FILECHOOSER_COMPUTER("computer"),
    FILECHOOSER_FLOPPY("floppy"),
    FILECHOOSER_NEW_FOLDER("folder"),
    FILECHOOSER_PARENT("go-up"),
    FILECHOOSER_DETAILS_VIEW("view_details"),
    FILECHOOSER_LIST_VIEW("view_list"),
    FILECHOOSER_VIEW("view_list"),
    FILECHOOSER_MUSIC("audio"),
    FILECHOOSER_DOWNLOADS("download"),
    FILECHOOSER_VIDEO("video"),
    FILECHOOSER_IMAGES("image"),
    FILECHOOSER_BOX("box"),
    FILECHOOSER_DOCUMENTS("document"),
    FILECHOOSER_HOME("go-home"),
    TABLE_LOCK_COLUMN("exttable/lockColumn"),
    TABLE_FINDMENU("exttable/findmenu"),
    TABLE_RESET_COLUMNS("exttable/resetColumns"),
    HEADLESS_checkbox_true("checkbox_true"),
    HEADLESS_checkbox_false("checkbox_false"),
    DIALOG_HELP("dialog/help"),
    DIALOG_ERROR("dialog/error"),
    DIALOG_WARNING("dialog/warning"),
    trayicon("trayicon");
    // *
    // */
    // public static AWIcon AWIcon.FILECHOOSER_FOLDER = AWIcon.FILECHOOSER_folder;
    // public static final String AWIcon.FILECHOOSER_HOME = "home";
    // public static final String AWIcon.FILECHOOSER_DOCUMENTS = "document";
    // public static final String AWIcon.FILECHOOSER_MUSIC = "audio";
    // public static final String AWIcon.FILECHOOSER_VIDEO = "video";
    // public static final String AWIcon.FILECHOOSER_BOX = "box";
    // public static final String AWIcon.FILECHOOSER_IMAGES = "image";
    // public static final String AWIcon.FILECHOOSER_DOWNLOADS = "download";
    private String path;

    /**
     *
     */
    private AWIcon(String path) {
        this.path = path;
    }

    /**
     * @param i
     * @return
     */
    public Icon get(int size) {
        return AWUTheme.I().getIcon(path, size);
    }

    public Icon icon(int size) {
        return AWUTheme.I().getIcon(path, size);
    }

    /*
     * (non-Javadoc)
     *
     * @see org.appwork.resources.IconRef#image(int)
     */
    @Override
    public Image image(int size) {
        return AWUTheme.I().getImage(path, size);
    }

    /*
     * (non-Javadoc)
     *
     * @see org.appwork.resources.IconRef#path()
     */
    @Override
    public String path() {
        // TODO Auto-generated method stub
        return path;
    }
}
